...
const OrderMenu: FC<Props> = function ({ setBottomSheetState }) {
    ...
    const memoSnapPoints = useMemo(() => getBottomSheetSnapPoints(), []);

    useEffect(() => {
        const updateSnapPoints = () => {
            const points = memoSnapPoints[BottomSheetStateEnum.SET_ADDRESS];
            ...
            if (isLoading) {
                const updatedPoints = points.map((pos) => pos + 300);
                snapToPosition(300);
                handleSetSnapPoints(updatedPoints);
                handleSetIndex(0);
                setSnapPos(300);
            }
        };
        ...
        const handleCreateOrders = async () => {
            try {
                if (
                    ...
                    console.log("get price", response.data);
                } else {
