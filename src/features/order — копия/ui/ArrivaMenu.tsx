...
const ArriveMenu: FC<Props> = function ({ setBottomSheetState }) {
    ...
    const [snapPos, setSnapPos] = useState(
        BOTTOM_SHEET_SNAP_POINTS[BottomSheetStateEnum.SET_ARRIVAL_LOCATION][0]
    );
    useEffect(() => {
        const points =
            BOTTOM_SHEET_SNAP_POINTS[BottomSheetStateEnum.SET_ARRIVAL_LOCATION];
        if (order.additionalArrivals.length === 0) {
            snapToPosition(points[0]);
        } else if (order.additionalArrivals.length == 1) {
            snapToPosition((points[0] = 470));
            getBottomSheetSnapPoints();
            setBottomSheetSnapPoint(
                BottomSheetStateEnum.SET_ARRIVAL_LOCATION,
                Platform.OS === "ios" ? [390] : [390]
            );
        } else if (order.additionalArrivals.length == 2) {
            snapToPosition((points[0] = 570));
            getBottomSheetSnapPoints();
            setBottomSheetSnapPoint(
                BottomSheetStateEnum.SET_ARRIVAL_LOCATION,
                Platform.OS === "ios" ? [590] : [590]
            );
        } else if (order.additionalArrivals.length == 3) {
            snapToPosition((points[0] = 670));
            getBottomSheetSnapPoints();
            setBottomSheetSnapPoint(
                BottomSheetStateEnum.SET_ARRIVAL_LOCATION,
                Platform.OS === "ios" ? [690] : [690]
            );
        }
    ...
    async function applyLocation() {
        try {
            ...
            for (let i = 0; i < stopsArr.length; i++) {
                if (stopsArr[i].city !== "" && stopsArr[i].address !== "") {
                    ...
                    if (points) {
                        ...
                        const additionalArrivals =
                            order.additionalArrivals?.slice() || [];
            ...
            handleSetOrder({
                ...order,
                arrival: editingOrder.arrival,
                additionalArrivals: [...stopsArr],
                newArrivals: [],
            });
        } catch (err) {
            console.error(
                "Failed to get geocode of additional arrival address: ",
                err
            );
        } finally {
            sheetModalRef.current?.snapToPosition(
                Platform.OS === "ios" ? 653 : 623
            );
    ...
    function handleDeleteAdditional(indexToDelete: number) {
        const additionalArrivals =
            order?.additionalArrivals
                ?.slice()
                .filter(
                    (_, indexAdditional) => indexAdditional !== indexToDelete
                ) || [];
        const removeMarker = order.additionalArrivals.find(
            (_, indexAdditional) => indexAdditional == indexToDelete
        );
