...
const DepartureCity: FC<Props> = function ({ setBottomSheetState }) {
    ...
    const [snapPos, setSnapPos] = useState(
        BOTTOM_SHEET_SNAP_POINTS[BottomSheetStateEnum.SET_DEPARTURE_CITY][0]
    );
    useEffect(() => {
        const points =
            BOTTOM_SHEET_SNAP_POINTS[BottomSheetStateEnum.SET_DEPARTURE_CITY];
    ...
    function handleSelectCity(selectedCity: string) {
        handleSetEditingOrder({
            ...editingOrder,
            departure: { ...editingOrder.departure, city: selectedCity },
        });
        setBottomSheetState(BottomSheetStateEnum.SET_DEPARTURE_LOCATION);
    }
